...
async def app(scope, receive, send):
    ...
    if path == "/v1/entities" and method == "GET":
        ...
        with psycopg.connect(DSN) as conn, conn.cursor() as cur:
            _json(send, 200, _entities(cur, kind, limit)); return
    if path.startswith("/v1/entities/") and method == "GET":
        try:
            cid = int(path.rsplit("/", 1)[-1])
        except ValueError:
            _json(send, 404, {"error": "not found"}); return
        with psycopg.connect(DSN) as conn, conn.cursor() as cur:
            ...
            _json(send, 200, {"id": r[0], "kind": r[1], "title": r[2], "normalized_title": r[3],
    ...
    if path == "/v1/sources" and method == "GET":
        with psycopg.connect(DSN) as conn, conn.cursor() as cur:
            ...
            _json(send, 200, [{"source": r[0], "count": r[1]} for r in cur.fetchall()]); return
    if path == "/v1/sync" and method == "POST":
        try:
            req = urllib.request.Request(f"{WORKER}/run", method="POST", data=b"")
            urllib.request.urlopen(req, timeout=5)
            _json(send, 202, {"sync": "kicked"}); return
        except urllib.error.URLError as e:
            _json(send, 502, {"error": f"worker unreachable: {e}"}); return
    _json(send, 404, {"error": "not found"})
